...
def _extract_series_data(series: list[dict], table_id: int) -> pd.DataFrame:
...
class INEExtractor:
    ...
    def get_maestro_provincias(self, n_ultimos: int = 1) -> pd.DataFrame:
        ...
        if not series:
            return pd.DataFrame()
        return _extract_series_data(series, _TABLA_POB_PROVINCIAS)
    ...
    def get_maestro_municipios(self, n_ultimos: int = 1) -> pd.DataFrame:
        ...
        if not series:
            return pd.DataFrame()
        return _extract_series_data(series, _TABLA_POB_MUNICIPIOS)
    ...
    def get_eoh_municipios(self, n_ultimos: int = 12) -> pd.DataFrame:
        ...
        if not series:
            return pd.DataFrame()
        return _extract_series_data(series, _TABLA_EOH_MUN_DEMANDA)
    ...
    def get_eoh_provincias(self, n_ultimos: int = 12) -> pd.DataFrame:
        ...
        if not series:
            return pd.DataFrame()
        return _extract_series_data(series, _TABLA_EOH_PROV_DEMANDA)
